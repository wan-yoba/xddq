...
export default class UnionMgr {
  ...
  pushMyUnionDataBroadcast(t) {
    logger.info(`妖梦原始数据 ${JSON.stringify(t)}`);
    ...
    GameNetMgr.inst.sendPbMsg(
      Protocol.S_WATCH_AD_TASK,
      { activityId: 0, conditionId: 120006, isUseADTime: false },
      null
    );
    GameNetMgr.inst.sendPbMsg(
      Protocol.S_TASK_GET_REWARD,
      { taskId: [120006] },
      null
    );
    logger.info("[妖盟管理] 妖盟买桃免费");
    GameNetMgr.inst.sendPbMsg(
      Protocol.S_MALL_BUY_GOODS,
      { mallId: 230000011, count: 1, activityId: 0 },
      null
    );
    logger.info("[妖盟管理] 妖盟买桃1");
    GameNetMgr.inst.sendPbMsg(
      Protocol.S_MALL_BUY_GOODS,
      { mallId: 230000001, count: 1, activityId: 0 },
      null
    );
    logger.info("[妖盟管理] 妖盟买桃2");
    GameNetMgr.inst.sendPbMsg(
      Protocol.S_MALL_BUY_GOODS,
      { mallId: 230000002, count: 1, activityId: 0 },
      null
    );
    logger.info("[妖盟管理] 妖盟买腾蛇信物");
    GameNetMgr.inst.sendPbMsg(
      Protocol.S_MALL_BUY_GOODS,
      { mallId: 230000012, count: 3, activityId: 0 },
      null
    );
    logger.info("[妖盟管理] 妖盟买腾蛇信物");
    GameNetMgr.inst.sendPbMsg(
      Protocol.S_UNION_GETDAILYTASK,
      { actIndex: 4 },
      null
    );
  }
  ...
  cutPriceSyncData(t) {
    if (t) {
      if (!t.records.find((record) => record.userId === UserMgr.playerId)) {
        ...
        GameNetMgr.inst.sendPbMsg(
          Protocol.S_SPIRIT_DRAW,
          { bussinessId: t.bussinessId },
          null
        );

        this.handleUnionTask();
        this.fightBoss();
      }
    }
  }
  ...
  fightBoss() {
    ...
    if (now - this.lastCheckTime >= this.CHECK_CD) {
      ...
      // TODO
      logger.info(
        "[妖盟管理] 妖盟讨伐 已满20人开始战斗，这是一个假的！还没有实现"
      );
