...
class MsgRecvMgr {
    ...
    static PlayerDataMsg(t) {
        ...
        UserMgr.playerBelong = t.playerBelong;
        UserMgr.unionId = t.unionId;
        UserMgr.wxHeadUrl = t.wxHeadUrl;
        UserMgr.serverId = Number(t.serverId);
        // 推送用户名姓名
        logger.info(`{"wxHeadUrl":"${t.wxHeadUrl}","nickName":"${t.nickName}","playerBelong":"${t.playerBelong}","playerId":"${t.playerId}"}`);
    }
    ...
    static GetSeparationDataMsgListResp(t) {
        ...
        logger.info(` 分身原始数据 ${JSON.stringify(t)} `);
    ...
    // 2025年2月12日 妖盟应该是变更了协议文件，暂时不不想弄。切换数据从 用户信息同步妖盟信息
